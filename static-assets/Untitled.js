...
(function() {
  var
    ...
    Self = null; // Local reference to CStudioAuthoring.ContextualNav.LayeredRootFolder initialized by CStudioAuthoring.register call
  ...
  Self = CStudioAuthoring.register({
    "ContextualNav.LayeredRootFolder": {
      ...
      treePaths: [],
      menuOn: false,
      treePathOpenedEvt: new YAHOO.util.CustomEvent("LayeredRootFolderTreePathLoaded", Self),
      labelsMenu: [],
      customIcons: {},
      defaultIcons: {
      ...
      /* =============================================== */
      /* Due to issues with Y.extend of wcm-root-folder this module is largely a copy of wcm-root folder
       * The following methods replace the wcm-root-folder methods and contain modifications / "Overrides"
       */
      ...
      initialize: function(config) {
        CStudioAuthoring.ContextualNav.LayeredRootFolder.configPayload = config;
              ...
              if (isComponent == true || isLevelDescriptor == true || isTaxonomy == true) {
                if (formPath == "" || formPath == undefined) {
                  ...
                  if (isUserAllowed) {
                    if (isDeleteAllowed) {
                      if(isOverlay) {
                        p_aArgs.addItems([ menuItems.removeOverlayOption ]);
                      }
                      else {
      ...
      onTriggerContextMenu: function(tree, p_aArgs) {
        ...
        var menuItems = {
          ...
          // THESE ARE THE ONLY TWO CHANGES IN THIS METHOD
          overlayOption: { text: "Override", onclick: { fn: Self.overlayContent } },
          removeOverlayOption: { text: "Remove Override", onclick: { fn: Self.deleteContent, obj:tree } }
      ...
      createTreeNodeTransferObject: function(treeItem) {
        ...
        // THIS IS THE ONLY LINE IN THIS METHOD THAT IS MODIFIED       
      ...
      drawSubtree: function(treeItems, root, pathToOpenTo, instance) {
        ...
        for (var i = 0, l = treeItems.length, treeNodeTO, renderChild; i < l; i++) {
          ...
          // THIS IS THE ONLY MOD IN THIS METHOD
      ...
      lookupSiteContent: function(site, path, level, action, callback) {
        this.lookupLayeredSiteContent(CStudioAuthoringContext.site, path, level, "default", callback);
      },
...
// END OF OVERRIDES 
      ...
      /* The following methods are layered-root folder specific */
      ...
      getLangFromPath(path) {
        return ((path.match(/(\/ja|\/en)$/) || [])[0] || '').substr(1);
      },
      ...
      /* ========================================== */
// END OF LAYERED ROOT Folder Methods
// All code below this point is copied as-is from wcm-root-folder 
      /* ========================================== */
      ...
      addContentTreeRootFolder: function(instance) {
        ...
        var $langSelector =  $('<select><option value="en">EN</option><option value="ja">JA</option></select>');
        ...
        $langSelector.appendTo(parentFolderLinkEl);
        ...
        if (this.pathOnlyHasCannedSearch(rootPath, instance)) {
          ...
          (function (ind) {
            function treePrint(servPath) {
              Self.lookupSiteContent(site, servPath, 1, "default", {
      ...
      onLoadNodeDataOnClick: function(node, fnLoadComplete) {
        ...
        Self.lookupSiteContent(site, path, 1, "default", serviceCb);
      },
                ...
                if(nodeToChange){
                  for(var i=0; i<nodeToChange.length;i++) {
                    (function (nodeToChange,i) {
                      Self.lookupSiteContent(nodeToChange[i], currentUri);
                      nodeOpen = true;
                    })(nodeToChange,i);
                  }
                }
                        ...
                        for (var j = 0; j < treeToUpdateDependencies.length; j++) {
                          if(treeToUpdateDependencies[j].data.contentType != "folder"){
                            (function (treeToUpdateDependencies, j) {
                              Self.lookupSiteContent(treeToUpdateDependencies[j], treeToUpdateDependencies[j].data.uri);
                              nodeOpen = true;
...
// LOOKUP SITE CONTEXT WAS DEFINED HERE
              }
            }else {
              if(node) {
                nodeOpen = true;
        ...
        var editCb = {
          success: function(contentTO, editorId, name, value, draft) {
            ...
            if(contentTO.updatedModel && contentTO.initialModel &&
              ...
              if(CStudioAuthoring.ContextualNav.LayeredRootFolder) {
                var tree = CStudioAuthoring.ContextualNav.LayeredRootFolder.myTree
  ...
  CStudioAuthoring.ContextualNav.LayeredRootFolderInstance = function(config) {
    ...
    if (!CStudioAuthoring.ContextualNav.LayeredRootFolderInstance.configPayload) {
      CStudioAuthoring.ContextualNav.LayeredRootFolderInstance.configPayload = {};
    }

    CStudioAuthoring.ContextualNav.LayeredRootFolderInstance.configPayload[config.params.path] = config;



    $(config.containerEl).html('');
    ...
    if(config.params.path === '/site/website'){
      amplify.subscribe("SELECTED_CONTENT_SET", function (message) {
        ...
        if (!highlightVisible && treeExists && reload) {
          ...
          CStudioAuthoring.ContextualNav.LayeredRootFolder.initialize(Object.assign({}, config, { containerEl: $container[0] }));
        } else {
          $('#acn-dropdown-menu [data-uri]').removeClass('highlighted');
          $highlightEl.addClass('highlighted');
          CStudioAuthoring.ContextualNav.LayeredRootFolder.scrollToHighlighted();
        }
      });
    }
  }
